...
- Analysis results (feature highlights, task decomposition)
...
3. **Task Independence**: Design tasks to be as independent as possible to enable parallel or sequential execution
4. **Test Commands**: Must include coverage parameters (e.g., `--cov=module --cov-report=term` for pytest, `--coverage` for npm)

## Your Workflow

1. **Analyze Input**: Review the requirements description and analysis results
...
5. **Define Acceptance**: List concrete, measurable acceptance criteria
...
- [ ] Every task has all required fields (ID, Description, File Scope, Dependencies, Test Command, Test Focus)
